...
ex:modul
    ...
    ex:raumS "z333" ;
    ex:raumP "Z407" ;
    ex:technikV "Beamer/WLAN" ;
    ex:technikS "Beamer/WLAN" ;
    ex:technikP "Beamer" .
...
// --- Admin login UI and handlers (appended) ---
(function(){
  // inject simple login UI at top of page
  try {
    const container = document.createElement('div');
    container.id = 'admin-login';
    container.style = 'padding:8px;border:1px solid #ccc;margin-bottom:8px;display:flex;gap:8px;align-items:center;';
    container.innerHTML = `
      <input id="admin-user" placeholder="Admin user" style="padding:4px"/>
      <input id="admin-pass" type="password" placeholder="Passwort" style="padding:4px"/>
      <button id="admin-login-btn">Login</button>
      <button id="admin-logout-btn" style="display:none">Logout</button>
      <span id="admin-status" style="margin-left:8px;font-weight:600;"></span>
    `;
    const root = document.getElementById('controls') || document.body;
    root.insertBefore(container, root.firstChild);
  } catch(e){
    console.warn('Could not inject admin login UI', e);
  }

  async function refreshLists() {
    if (typeof loadSavedLists === 'function') {
      try { await loadSavedLists(); } catch(e) { console.warn('loadSavedLists error', e); }
    }
  }
  ...
  document.getElementById('admin-login-btn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (res.ok) {
        document.getElementById('admin-status').textContent = 'eingeloggt';
        document.getElementById('admin-login-btn').style.display = 'none';
        document.getElementById('admin-logout-btn').style.display = 'inline-block';
        await refreshLists();
      } else {
        const body = await res.json().catch(()=>({}));
        document.getElementById('admin-status').textContent = body.message || 'Login fehlgeschlagen';
      }
    } catch (err) {
      document.getElementById('admin-status').textContent = 'Fehler';
    }
  });
  ...
  document.getElementById('admin-logout-btn')?.addEventListener('click', async () => {
    ...
    document.getElementById('admin-status').textContent = 'ausgeloggt';
  ...
  // Intercept calls to loadSavedLists if it exists to show login hint on 401
  if (typeof loadSavedLists === 'function') {
    const _orig = loadSavedLists;
    window.loadSavedLists = async function() {
      try {
        return await _orig();
      } catch (err) {
        console.warn('loadSavedLists wrapper caught', err);
        throw err;
      }
    }
  }
